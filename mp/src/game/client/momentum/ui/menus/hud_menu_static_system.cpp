...
CON_COMMAND_F_COMPLETION(mom_hud_menu_show, "Opens a hud menu. Accepts the hud menu itself by file name. See cfg/menus/", 0, HudMenuCompletion)
{
    if (args.ArgC() > 1)
    {
...
void CHudMenuStaticSystem::ReloadMenuList()
{
    ...
    while (pFilename)
    {
        if (!FStrEq(pFilename, ".") && !FStrEq(pFilename, ".."))
        {
